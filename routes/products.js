 ...
 * /api/products/all:
 *   get:
 *     summary: Get all products without any filters
 *     tags: [Products]
 *     responses:
 *       200:
...
/**
 * @swagger
 * /api/products/{id}/check-availability:
 *   post:
 *     summary: Check if requested quantity is available
 *     tags: [Products]
 *     parameters:
 *       - in: path
 *         name: id
...
router.post('/:id/check-availability', async (req, res) => {
  try {
    ...
    const isAvailable = product.isQuantityAvailable(quantity);
    res.json({
      available: isAvailable,
      message: isAvailable ? 'Quantity is available' : 'Quantity is not available'
    });
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
